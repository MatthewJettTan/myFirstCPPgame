...
void PhysicalEntity::resolveConstrains(GameMap& mapData)
{
	...
	const float GRANULARITY = 0.8;	// predifined constant
	...
	else	// if the player move too far
	{
...
// Deal with only one direction at a time
Vector2 PhysicalEntity::performCollisionOnOneAxis(GameMap& mapData, Vector2 pos, Vector2 delta)
{
	...
	int minX = floor(pos.x - dimensions.x / 2 - 1);
	int maxX = ceil(pos.x + dimensions.x / 2 + 1);

	int minY = floor(pos.y - dimensions.y / 2 - 1);
	int maxY = ceil(pos.y + dimensions.y / 2 + 1);
	// make sure within the boundary of map
	minX = std::max(0, minX);
	minY = std::max(0, minY);
	maxX = std::min(maxX, mapData.w);
	maxY = std::min(maxY, mapData.h);

	// traverse the blocks on the map
